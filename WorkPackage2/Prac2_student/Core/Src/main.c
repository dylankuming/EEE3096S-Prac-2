...
uint8_t binaryValues[] = {0b10101010, 0b01010101, 0b11001100, 0b00110011, 0b11110000, 0b00001111};
uint16_t currentAddress = 0; // To keep track of the current EEPROM address or index in the array
int timlength=0;
...
int main(void)
{
  ...
  // TODO: Write all "binaryValues" to EEPROM using SPI
  for(uint16_t i = 0; i < sizeof(binaryValues); i++) {
  ...
  while (1)
  {
	...
	// TODO: Check button PA0; if pressed, change timer delay
	  if (HAL_GPIO_ReadPin(Button0_GPIO_Port, Button0_Pin) == GPIO_PIN_RESET) {
	      ...
	      timlength = !timlength;  // This toggles the value of timlength
	  }
...
void TIM16_IRQHandler(void)
{
	...
	uint8_t value = read_from_address(currentAddress);
	    // Assuming there's a function called setLEDs that displays values on LEDs
	if (value == binaryValues[currentAddress]){
		GPIOB->ODR = value;
	}else{
